...
function run_local_circuit_scan(n_qubit, resolution, local_hilbert_space_dimension=2, verbosity=0, save=true)
    if local_hilbert_space_dimension == 2
        eu_list = range(1/2, 2/3, resolution)
        gu_list = range(0.25, 0.75, resolution)
    elseif local_hilbert_space_dimension == 3
    ...
    res_list = map(iter) do item
        ...
        eu, gu = item
        res=diagonalize_local_circuit_arnoldi_matrix_free(n_qubit, local_hilbert_space_dimension, eu, gu)
        res[1]
    end
    ...
    if save
        npzwrite("localcircuit_eigvals_n=$(n_qubit)_res_$(resolution)_d_$(local_hilbert_space_dimension).npy", second_largest_eigenvalue)
        npzwrite("localcircuit_eulist_n=$(n_qubit)_res_$(resolution)_$(local_hilbert_space_dimension).npy", collect(eu_list))
        npzwrite("localcircuit_gulist_n=$(n_qubit)_res_$(resolution)_$(local_hilbert_space_dimension).npy", collect(gu_list))
    end
end
